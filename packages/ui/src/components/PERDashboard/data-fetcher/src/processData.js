  ...
  try {
      ...
      area_responses: result.area_responses.map((areaResponse) => ({
        ...
        component_responses: areaResponse.component_responses.map(
          (componentResponse) => {
            ...
            const urbanConsiderationsSimplified =
              containsAffirmativeWord(urban_considerations);
            const epiConsiderationsSimplified =
              containsAffirmativeWord(epi_considerations);
            const climateEnvironmentalConsiderationsSimplified =
              containsAffirmativeWord(climate_environmental_considerations);
    ...
    await fs.writeFile(
      ...
      JSON.stringify(
        {
          assessments: grouped,
          countryAssessments: countryAssessments,
        },
        null,
        2
      )
