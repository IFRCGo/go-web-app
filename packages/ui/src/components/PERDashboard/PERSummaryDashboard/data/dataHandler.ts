...
function assignFillColors(
  ...
  return data.map((record) => {
    const phaseMatch = PHASE_COLORS.find(
      (phase) =>
        phase.phase === record.phase_display &&
...
function applyFilters(
  ...
  if (filters.year) {
    ...
    filteredData = filteredData.filter(
      (record) =>
        new Date(record.date_of_assessment).getFullYear().toString() === yearStr
    );
  }
...
function getComponentSummaryForTreemap(
  ...
  filteredData.forEach((record) => {
    record.prioritized_components.forEach((component) => {
      ...
      let existingComponent = componentFrequency[areaTitle].children.find(
...
function getPERConsiderations(
  ...
  filteredData.forEach((record) => {
    ...
    if (record.epi_considerations) {
      considerations.epi_considerations[regionName][assessmentType]++;
      totalEpiConsiderations++;
    }

    // Climate Environmental Considerations
    if (record.climate_environmental_considerations) {
      considerations.climate_environmental_considerations[regionName][assessmentType]++;
      totalClimateConsiderations++;
    }

    // Urban Considerations
    if (record.urban_considerations) {
      considerations.urban_considerations[regionName][assessmentType]++;
      totalUrbanConsiderations++;
    }

    // Migration Considerations
    if (record.migration_considerations) {
      considerations.migration_considerations[regionName][assessmentType]++;
  ...
  const epiPercentage =
    totalAssessments > 0 ? (totalEpiConsiderations / totalAssessments) * 100 : 0;

  const climatePercentage =
    totalAssessments > 0 ? (totalClimateConsiderations / totalAssessments) * 100 : 0;

  const urbanPercentage =
    totalAssessments > 0 ? (totalUrbanConsiderations / totalAssessments) * 100 : 0;

  const migrationPercentage =
    totalAssessments > 0 ? (totalMigrationConsiderations / totalAssessments) * 100 : 0;
